...
export class Dashboard extends React.Component {
    ...
    toggleSidePane = (mode, action = 'open') => () => this.setState({
        showSidePane: !this.state.showSidePane,
        sidePaneMode: mode,
        selectedBusiness: action === 'close'
        ? {
            category: '',
            created_by: '',
            id: '',
    ...
    toggleBusinessDropdown = (businessId) => () => this.setState({
        clickedBusinessId: this.state.clickedBusinessId !== businessId ? businessId : null
    })

    handleBusinessDropdownButton = (business, toggleMethod) => () => this.setState({
        selectedBusiness: business,
        clickedBusinessId: null
    }, () => toggleMethod())

    deleteConfirmedBusiness = () => this.props.deleteBusiness(
        this.accessToken, this.state.selectedBusiness.id
                        ...
                        {this.state.pageOfItems.map(business => {
                            return (
                                <BusinessCard
                                    ...
                                    onClickEllipsisHandler={this.toggleBusinessDropdown(business.id)}
                                    onClickDropdownEditButton={this.handleBusinessDropdownButton(business, this.toggleSidePane('Edit'))}
                                    onClickDropdownDeleteButton={this.handleBusinessDropdownButton(business, this.toggleDeleteModal)}
                                />
                            );
                        })}
                    </div>
...
const mapDispatchToProps = (dispatch) => ({
    ...
    updateBusiness: (accessToken, id, newData) => dispatch(updateBusiness(accessToken, id, newData))
});
